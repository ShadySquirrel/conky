...
key = 'no-key-grab-your-own'
# list of cities. Left Belgrade here, for fun.
cities = ['792680']
# where to store data. Set your path!
...
for city in cities:
	...
	to_print += "${voffset 10}${goto 35}${color}Currently: ${color1}%s, %.1f°C ${color}(${color1}%.1f°C ${color}min, ${color1}%.1f°C${color} max)\n" % (description['main'], toCelsius(weather['temp']), toCelsius(weather['temp_min']), toCelsius(weather['temp_max']))
	...
	to_print += "${voffset 5}${goto 35}${alignr 0}${font Fira Sans:size=6.5}Data from ${color1}%s${color}${font}\n" % timestamp2String(int(weather_data['dt']))
	
	to_print += "${voffset -5}${goto 35}${alignr 0}${font Fira Sans:size=6.5}Refreshed at ${color1}%s${color}${font}\n" % timestamp2String(int(weather_data['refresh_time']))
